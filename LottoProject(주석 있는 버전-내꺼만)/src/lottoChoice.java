...
////////////////////////////////////////////////////로또 번호 정하는 파트//////////////////////////////////////////////
		...
		for (int i = 0; i < lblAll.length; i++) {
			lblAll[i] = new JLabel((ImageIcon) NumberToImages.getInstance().getNumberToImage().get(i + 1));	// 0부터 44까지 되기때문에 +1을 했다.
			lblAll[i].setText("" + (i + 1));
			...
			lblAll[i].addMouseListener(new MouseAdapter() {
				...
				public void mousePressed(MouseEvent e) {
					...
					int clickNumbers = Integer.parseInt(clickLabel.getText());
					...
					if (choiceLottoNumbers.size() < 7 && UserGetLottoNumbers.size() < 7) {	//	6개 다찼을때는 조건문이 바로 나가져서 Label 취소가 안된다. 그래서 size를 6에서 7로 바꾸었다. 번호 추가는 6개만 되게 했다.
						...
						for (int i = 0; i < lblAll.length; i++) {
							...
							if (clickLabel.equals(lblAll[i])) { // 클릭 이벤트가 발생한 객체의 source와 label이미지객체의 source가 같으면 click 횟수가 증가한다. 나중에 삭제할때 쓸려고
								clickcount++;					// 횟수 알게했다.
								System.out.println("클릭한 이미지 번호 = " + clickNumbers + ", 클릭 횟수 = " + clickcount);
							}
						}
						if (clickcount % 2 != 0 && choiceLottoNumbers.size() < 6) {
							lblAll[clickNumbers - 1].setIcon((ImageIcon) NumberToImages.getInstance().getNumberToImage().get(clickNumbers + 45));	//수정 할꺼!!!!
							choiceLottoNumbers.add(clickNumbers); // set을 사용한 이유는 제거함에 있어서 원소값만 제거하면 되므로 과정이 더 편해서 이걸
																	// 선택했다.
						} else if (clickcount % 2 == 0) {	// 짝수번 클릭했을때 그 원소 삭제하게 했다.
							choiceLottoNumbers.remove(clickNumbers);
							System.out.println("삭제됨!");
							lblAll[clickNumbers - 1].setIcon((ImageIcon) NumberToImages.getInstance().getNumberToImage().get((clickNumbers)));
						} else {
		...
		autoButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				while (UserGetLottoNumbers.size() < 6) {
					...
					lblAll[random - 1].setIcon((ImageIcon) NumberToImages.getInstance().getNumberToImage().get((random) + 45));
					Collections.sort(UserGetLottoNumbers);
				}
...
//				}
		...
		checkButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				if (UserGetLottoNumbers.size() == 6) {
					for (int i = 0; i < 6; i++) {
						UserNumberImage[i].setIcon((ImageIcon)NumberToImages.getInstance().getNumberToImage().get(UserGetLottoNumbers.get(i)));		// URL에서 숫자 0 이 이미지 1이다. 즉 숫자와 이미지가 완전히 대응되는게 아닌 이미지가 숫자 + 1의 값이 되었다.
					}
	...
	//				choiceLottoNumbers.remove(choiceLottoNumbers);
					...
					for (int i = 0; i < UserGetLottoNumbers.size(); i++) {	// clear하고 난 뒤 이미지를 지우면 데이터 값이 없으므로 index가 없어 index 오류가 나타난다.
						lblAll[UserGetLottoNumbers.get(i) - 1].setIcon((ImageIcon) NumberToImages.getInstance().getNumberToImage().get(UserGetLottoNumbers.get(i)));
